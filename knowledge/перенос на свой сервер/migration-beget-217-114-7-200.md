...
## Важно на будущее: права файлов после scp

Сервис работает от www-data и статика лежит с правами `-rw------- www-data:www-data`
(600, не 644 - строже обычного, но так уже было настроено). При копировании новых
файлов через `scp` с ключом агента они приходят как `root:root 600` - www-data не
может их прочитать (EACCES в логах, страница отдаёт 500 только на этот файл, HTML
при этом грузится нормально). После КАЖДОГО scp новых статических файлов (картинки,
и т.п.) обязательно: `chown www-data:www-data <файл> && chmod 600 <файл>` - иначе
100% поймаешь эту же ошибку снова. Столкнулись 04.09.2026 при заливке 3 сертификатов.
